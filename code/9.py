...
def UndoNine(iA, iB, iC, iD, iE, iF):
...
def RedoNine(iA, iB, iC, iD, iE, iF):
	A = iA 
	B = iB
	C = iC
	D = iD
...
def RedoUndoNine(iA, iB, iC, iD, iE, iF):
	A = iA 
	B = iB
	C = iC
	D = iD
	...
	f1 = open('../logs/9.txt_undoredo','wa+')
	Output = []
	Output.append("<start T1> " + printVars(A,B,C,D,E,F))
	t1=iA
	t1 = t1*2
...
UndoNine(8,8,5,10,5,10)
RedoNine(8,8,5,10,5,10)
RedoUndoNine(8,8,5,10,5,10)
