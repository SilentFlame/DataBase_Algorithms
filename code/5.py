...
def UndoFive(iA, iB, iC, iD, iE, iF):
...
def RedoFive(iA, iB, iC, iD, iE, iF):
	A = iA 
	B = iB
	C = iC
	D = iD
...
def RedoUndoFive(iA, iB, iC, iD, iE, iF):
	A = iA 
	B = iB
	C = iC
	D = iD
	...
	f1 = open('../logs/5.txt_undoredo','wa+')
	Output = []
	Output.append("<start T1> " + printVars(A,B,C,D,E,F))
	t1=iA
	t1 = t1*2
...
UndoFive(8,8,5,10,5,10)
RedoFive(8,8,5,10,5,10)
RedoUndoFive(8,8,5,10,5,10)
