...
def UndoThree(iA, iB, iC, iD, iE, iF):
...
def RedoThree(iA, iB, iC, iD, iE, iF):
	A = iA 
	B = iB
	C = iC
	D = iD
...
def RedoUndoThree(iA, iB, iC, iD, iE, iF):
	A = iA 
	B = iB
	C = iC
	D = iD
	...
	f1 = open('../logs/3.txt_undoredo','wa+')
	Output = []
	Output.append("<start T1> " + printVars(A,B,C,D,E,F))
	t1=iA
	t1 = t1*2
...
UndoThree(8,8,5,10,5,10)
RedoThree(8,8,5,10,5,10)
RedoUndoThree(8,8,5,10,5,10)
